...
object Integer {
  ...
  def fromSignAndNatural(sign: Sign, value: Natural): Integer = (sign, value) match {
    case (Sign.Zero, _) => Zero
    case (Sign.Positive, Natural.Zero) => Zero
    case (Sign.Negative, Natural.Zero) => Zero
      ...
      bits match {
        case Nil => 
          val quotient = quotientBits.reverse match {
            ...
            case qBits => Natural.Positive(qBits)
          }
  ...
  private def shiftAndAdd(n: Natural, bit: Boolean): Natural = n match {
    case Natural.Zero => if (bit) Natural.Positive(List(true)) else Natural.Zero
    case Natural.Positive(bits) => 
      val newBits = bit :: bits
      Natural.Positive(newBits)
  }
  ...
  def power(base: Integer, exponent: Natural): Integer = exponent match {
    ...
    case Natural.Positive(List(true)) => base // exponent == 1
    case _ => 
      // 二進法によるべき乗
  ...
  private def shiftRight(n: Natural): Natural = n match {
    ...
    case Natural.Positive(List(_)) => Natural.Zero
    case Natural.Positive(_ :: tail) => 
      if (tail.isEmpty) Natural.Zero
      else {
        val trimmed = tail.reverse.dropWhile(!_).reverse
        if (trimmed.isEmpty) Natural.Zero else Natural.Positive(trimmed)
      }
  }
  ...
  def compare(a: Integer, b: Integer): Int = (a, b) match {
    ...
    case (Negative(av), Negative(bv)) => Natural.compare(bv, av) // 符号が逆
  }
